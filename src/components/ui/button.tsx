...
const buttonVariants = cva(
  "group/button inline-flex shrink-0 items-center justify-center border bg-clip-padding whitespace-nowrap outline-none select-none transition-colors disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0",
  {
    variants: {
      variant: {
        primary:
      ...
      size: {
        ...
        map: "size-10 rounded-lg p-2 [&_svg:not([class*='size-'])]:size-4",
      },
    },
    defaultVariants: {
      variant: "primary",
...
function Button({
  ...
  const resolvedSize = size ?? (variant === "map" ? "map" : "default")
